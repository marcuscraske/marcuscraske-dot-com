...
${append_js("https://cdn.mathjax.org/mathjax/latest/MathJax.js?config=TeX-AMS-MML_HTMLorMML")}

${append_js("/content/syntaxhighlighter/scripts/shCore.js")}
${append_js("/content/syntaxhighlighter/scripts/shBrushCpp.js")}
${append_js("/content/syntaxhighlighter/scripts/shBrushPlain.js")}
${append_css("/content/syntaxhighlighter/styles/shCore.css")}
${append_css("/content/syntaxhighlighter/styles/shThemeDefault.css")}
...
<h3>Using a Basic Shift Register</h3>
<h4>Hardware Communication</h4>
<p>
    Due to the number of limited outputs on the Raspberry Pi, it's practical to use a <i>shift-register</i> to virtually increase the number of outputs; for a basic 24-hour binary clock, you will require at least 17 LEDs - which would consume nearly all of the usable output ports on the Raspberry Pi.
</p>
<p>
    The following section is about the 74HC595, an integrated-circuit with 16 pins; the 74HC595 is practical because eight pins can be utilized as output's and multiple IC's can be connected in series - it's also very cheap. To control 17 LEDs to display binary time, we shall use three shift registers (allowing for up to 24 outputs)...which requires <b>only three output pins</b> from our Raspberry Pi.
</p>
<p>
    Pin layout:
</p>
...
<ul>
    ...
    <li>
        Toggle the clock by setting it to high and then setting it back to low; this will cause the shift register to set the pin either on/off based on the data-pin.
    </li>
    <li>
...
<p>
    When we want to control more than eight pins, i.e. multiple shift-registers in-series, we simply repeat step 2 (setting which pins to be on/off) sixteen times. However each shift-register in-series is connected like a chain; every shift-register after the first has its data-pin connected to the previous shift-register's overflow pin (refer to pin-layout). Therefore when the first eight pins have been set on the primary shift-register, the data signal is pushed to the overflow pin - which goes to the data-pin of the next/second shift-register; this continues until the latch is set back to high at the end.
</p>
...
<p>
    This diagram may look scary at first, however follow the circuit for the data-pin and re-read the above paragrah - you should see that the primary/first shift-register is the right-most shift-register.
</p>

<h4>Software Communication</h4>
<p>
...
<p>
    I do not have the actual code available for the above test, due to corruption with the Debian image; however the code used the <i>shiftOut</i> function of the WiringPi library, which worked something like this:
</p>
...
<h4>Hardware Communication</h4>
<p>
    Since I decided to use a photo-resistor and TMP36 temperature sensor, we require analogue inputs to read their values; however the Raspberry Pi does not have any analogue inputs (only digital / high and low). Therefore the solution is to use an analogue-to-digital converter, which translates an analogue signal (0 to 1023 - since the MCP3008 has a 10-bit resolution / 2^10-1=1023 ~ we subtract one since we start at zero of-course) to on/off (compatible with our Raspberry Pi); I have decided to use the MCP3008 because it's fairly simple to use.
</p>
...
<ul>
    <li>
        <b>Clock</b> - used to specify when the data-in pin has been set; the same principal as the clock from the shift-registers previously mentioned; this will also be used to read bits out as well.
    </li>
    <li>
        <b>Data-out</b> - used for reading bits, or rather high's and/or low's, which represent the numeric value of a pin (from 0 to 1023 due to being analogue).
    </li>
    <li>
        <b>Data-in</b> - used to specify which pin's value to output on the data-out pin.
    </li>
    <li>
        <b>Chip-select</b> - a similar function to the latch of a shift-register in this context, click <a href="http://en.wikipedia.org/wiki/Chip_select">here</a> for more information.
    </li>
</ul>
...
<h5>Transmitting the Pin to be Read</h5>
<p>
...
<h5>Reading the Value of the Pin</h5>
<p>
...
<p>
    During each read, we will toggle the clock (set it to high and then low) to inform the MCP3008 we have read the <i>data-out</i> pin.
</p>
...
<p>
    If you're still unsure, I would also recommend reading the article by <a href="http://jeremyblythe.blogspot.co.uk/2012/09/raspberry-pi-hardware-spi-analog-inputs.html">Jeremy Blythe</a> from his article on the MCP3008.
</p>

<h4>Hardware Sensors</h4>
<p>
    I am using the MCP3008 to read the temperature and the amount of light using a photo-resistor; the circuit looks like the following:
</p>
...
<h5>Photo-Resistor - Light Sensor</h5>
<p>
...
<h5>TMP36 - Temperature Sensor</h5>
<p>
...
<h4>Constructing &amp; Debugging Physical Circuit Boards</h4>
<p>
